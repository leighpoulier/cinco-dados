...
describe "Game Model" do
    ...
    it "detects straights" do
        expect(GameModel.calculate_scores([1,2,3,4,5])).to eq({
        ...
        expect(GameModel.calculate_scores([2,3,4,5,6])).to eq({
    ...
    it "detects full house and three of a kind" do
        expect(GameModel.calculate_scores([1,1,1,2,2])).to eq({
        ...
        expect(GameModel.calculate_scores([6,5,6,5,6])).to eq({
    ...
    it "detects three of a kind and four of a kind" do
        expect(GameModel.calculate_scores([1,2,1,1,1])).to eq({
        ...
        expect(GameModel.calculate_scores([3,3,4,3,3])).to eq({
    ...
    it "detects large straights" do
        expect(GameModel.calculate_scores([6,2,4,3,5])).to eq({
            ones: 0, twos: 2, threes: 3, fours: 4, fives: 5, sixes: 6,
            three_of_a_kind: 0, four_of_a_kind: 0, full_house: 0,
            small_straight: 30, large_straight: 40, cinco_dados: 0, chance: 20
        })
        expect(GameModel.calculate_scores([5,2,1,4,3])).to eq({
            ones: 1, twos: 2, threes: 3, fours: 4, fives: 5, sixes: 0,
            three_of_a_kind: 0, four_of_a_kind: 0, full_house: 0,
    ...
    it "detects cinco dados" do
        expect(GameModel.calculate_scores([1,1,1,1,1])).to eq({
        ...
        expect(GameModel.calculate_scores([4,4,4,4,4])).to eq({
    ...
    it "gives exceptions for bad dados array data" do
        expect { GameModel.calculate_scores([]) }.to raise_error(CincoDados::DadosError)
        expect { GameModel.calculate_scores(["a", "b", "c"]) }.to raise_error(CincoDados::DadosError)
        expect { GameModel.calculate_scores("String") }.to raise_error(CincoDados::DadosError)
        expect { GameModel.calculate_scores(5) }.to raise_error(CincoDados::DadosError)
        expect { GameModel.calculate_scores(8.23) }.to raise_error(CincoDados::DadosError)
        expect { GameModel.calculate_scores({test: "hash"}) }.to raise_error(CincoDados::DadosError)
    end         
end